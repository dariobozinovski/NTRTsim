...
for file_name in all_files:
    ...
    if(extensionRate):
        ...
        for i,curr_col in enumerate(curr):
            rest_col = rest[i]
            # Calculate the extension
            difference=df[curr_col]
            extension.append(difference)
        for spring, (style, color) in zip(range(1, len(extension)+1), zip(cycle(line_styles), colors)):
...
#if waned plot the position of the robot
    if positionplot:
        # Normalize time data
        time_min = df['time'].min()
        time_max = df['time'].max()
        df['normalized_time'] = (df['time'] - time_min) / (time_max - time_min)
        ...
        x_pos = df[x].mean(axis=1) * 0.1
        y_pos = df[y].mean(axis=1) * 0.1
        z_pos = df[z].mean(axis=1) * 0.1

        # Compute y-acceleration using the right timestep
        y_acc = np.gradient(np.gradient(y_pos, df['time']), df['time'])

        # Define color based on acceleration
        acceleration_threshold = -9.81
        margin = 0.1  # Define your acceptable margin
        color_values = np.where((y_acc >= acceleration_threshold - margin) & (y_acc <= acceleration_threshold + margin),
                            1.0,  # Red when within threshold
                            df['normalized_time'])  # Use normalized time elsewhere

        # Map color values to colorscale properly
        colorscale = [(0, 'blue'),(0.5, 'green'), (0.999, 'yellow'), (1.0, 'red')]
        ...
        fig.add_trace(go.Scatter3d(
            ...
            marker=dict(
                ...
                colorscale=colorscale,  # From blue to yellow
                colorbar=dict(title='Normalized Time'),
                showscale=True
            ),
            text=[f"X: {xi:.2f}, Y: {yi:.2f}, Z: {zi:.2f}, Acc: {ai:.2f} m/s², Time: {ti:.2f} s"
                for xi, yi, zi, ai, ti in zip(x_pos, y_pos, z_pos, y_acc, df['time'])],  # Hover text
            hoverinfo='text'
        ))
        ...
        fig.update_layout(
            ...
            title='3D Scatter Plot of Rods with Acceleration Color Coding'
        )
        ...
        plot1_path = os.path.join(new_plots_directory_path, "position_plot.html")
        fig.write_html(plot1_path)

    #new plot where we put accelarion in y with time with same color coding
    if roddistance:
        # Define columns for X, Y, Z coordinates
        x = [col for col in df.columns if 'prism_rod' in col and col.endswith('.X')]
        y = [col for col in df.columns if 'prism_rod' in col and col.endswith('.Y')]
        ...
        # Calculate the average position of all rods
        x_pos = df[x].iloc[1] * 0.1
        y_pos = df[y].iloc[1] * 0.1
        z_pos = df[z].iloc[1] * 0.1

        # print the position with the name of the rod
        print("position of the rods")
        for i in range(num_rods):
            print(f"rod {i} x: {x_pos.iloc[i]} y: {y_pos.iloc[i]} z: {z_pos.iloc[i]}")
        rod1=0
        while (rod1!=-1):
            # ask for rods to calculate
            rod1=int(input("rod1:"))
            rod2=int(input("rod2:"))
            # Calculate the distance between two rods at last time step

            if(rod1!=-1): 
                distance = np.sqrt((x_pos.iloc[rod1] - x_pos.iloc[rod2])**2 +
